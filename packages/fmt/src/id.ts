...
const regex =
  /^\s*@@id\((?<prefix>fields\s*:\s+)?\[(?<fields>.+)\]\s*(?<attributes>.+?)?\s*\)\s*$/u;

export const isId = (line: string): boolean => regex.test(line);

export class Id implements Line {
  ...
  constructor(line: string) {
    ...
    if (!groups) {
      throw new Error(`Expect: "@@id". Actual: "${line}"`);
    }
