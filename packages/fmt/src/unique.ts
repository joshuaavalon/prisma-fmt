...
const regex =
  /^\s*@@unique\((?<prefix>fields\s*:\s+)?\[(?<fields>.+)\]\s*(?<attributes>.+?)?\s*\)\s*$/u;

export const isUnique = (line: string): boolean => regex.test(line);

export class Unique implements Line {
  ...
  constructor(line: string) {
    ...
    if (!groups) {
      throw new Error(`Expect: "@@unique". Actual: "${line}"`);
    }
